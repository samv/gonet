...
type Ping_Manager struct {
	...
	output map[ipv4tps.IPhash](ipv4.Writer)
...
func NewPing_Manager(icmprm *icmp.ICMP_Read_Manager) (*Ping_Manager, error) {
	...
	pm := &Ping_Manager{
		input:             input,
		output:            make(map[ipv4tps.IPhash](ipv4.Writer), 1),
		reply:             reply,
...
func (pm *Ping_Manager) getIP_Writer(ip *ipv4tps.IPAddress) (ipv4.Writer, error) {
	if x, ok := pm.output[ip.Hash()]; ok {
		return x, nil
	}
	wt, err := ipv4.NewIP_Writer(ip, ipv4.IPProtoICMP)
	if err != nil {
		return nil, err
	}
